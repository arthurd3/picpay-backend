...
public class ParkingSpotController {
    ...
    //NAO PRECISOU DEFINIR URI POIS JA FOI DEFINIDO A NIVEL DE CLASSE "/parking-spot"
    //SEMPRE QUE UM CLINTE PRECISAR FAZER UM POST O METODO SABEPARKINGSPOT SERA ACIONADO
    @PostMapping
    public ResponseEntity<Object> saveParkingSpot(@RequestBody @Valid ParkingSpotDto parkingSpotDto) {
        //VERIFICA SE JA HA ALGUM REGISTRO
        if(parkingSpotService.existsByLicensePlateCar(parkingSpotDto.getLicensePlateCar())){
            return ResponseEntity.status(HttpStatus.CONFLICT).body("Conflict: License Plate Car is already in use!");
        }
        //VERIFICA SE JA HA ALGUM REGISTR
        ...
        //REALIZA CONVERSAO , DE DTO PARA MODEL
        BeanUtils.copyProperties(parkingSpotDto, parkSpotModel);
        parkSpotModel.setRegistrationDate(LocalDateTime.now(ZoneId.of("UTC")));
